...
int main( void )
{
  ...
  {
    commande[ 0 ] = '\0';
    fgetstring( 64 , commande );
    if ( strlen( commande ) != 0 )
    {
      if ( strcmp( commande , "DISPLAY GRID" ) == 0 )
      {
        display_grid();
      }
      else if ( strcmp( commande , "DISPLAY WORDS" ) == 0 )
      {
        display_words();
      }
      ...
      {
        zmq_send( client_data->socket->socket_port1 , "" , 0 , ZMQ_SNDMORE );
        zmq_send( client_data->socket->socket_port1 , commande , 64 , 0 );
      }
    }
  }
  zmq_send( client_data->socket->socket_port1 , "" , 0 , ZMQ_SNDMORE );
