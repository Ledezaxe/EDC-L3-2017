...
char *format_msg(char *word, int x, int y, int ori)
{
  ...
  size = strlen(word) + sprintf(s_xf, "%d", xf) + sprintf(s_yf, "%d", yf) + sprintf(s_x, "%d", x) + sprintf(s_y, "%d", y) + 5 + 6 + 1 + 10;
...
char *force_brute(char **map, char **words,int xmax, int ymax)
{
      ...
      while (!found && y < ymax)
	{
	  while (!found && words[i] != NULL)
		...
		if (words[i][0] == map[x][y])
		{
		  ...
		  if (found != 0)
		    return (format_msg(words[i], x, y++, found));
		}
	      i++;
	    }
	  i = 0;
